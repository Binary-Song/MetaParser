...
public:
    ...
    /// 给定语言的语法规则，初始化语法分析器。
    /// @param compute_pred_tbl 若为false，将不会计算各种集合，适合手动输入first,follow和select集合。否则将调用compute_first_set等函数计算first,follow和select集合。
    Parser(Rules const &rules, bool compute_pred_tbl = true)
        : rules(rules)
    {
        if (compute_pred_tbl)
        {
